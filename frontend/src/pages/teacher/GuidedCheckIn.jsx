...
import { demoStudents, checkInPrompts, doTips, dontTips, observeNextTips } from '@/lib/demoData';
        ...
        <CardContent className="p-5">
          ...
          <div className="space-y-2">
            {checkInPrompts.map((prompt, i) => (
              <div key={i} className="flex items-start gap-3 p-3 rounded-lg bg-secondary/50 group">
        ...
        <CardContent className="p-5">
          ...
          <div className="space-y-2">
            {(student.key_factors.length > 0
              ? observeNextTips[student.key_factors[0]?.category] || observeNextTips.general
              : observeNextTips.general
            ).map((tip, i) => (
            ...
            {observeNextTips.general.map((tip, i) => (
              <div key={`g${i}`} className="flex items-start gap-2 text-xs text-sky-700/70">
                <span className="shrink-0 mt-0.5">→</span>
                <span>{tip}</span>
              </div>
            ))}
